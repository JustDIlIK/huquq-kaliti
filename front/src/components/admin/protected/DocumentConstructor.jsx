...
import {createDocument} from "../../../store/documents/documentsActions.js";
...
const DocumentConstructor = () => {
    ...
    useEffect(() => {
        const savedDocument = localStorage.getItem('huquq-document');
        if (savedDocument) {
            let savedDocumentParse = JSON.parse(savedDocument)
            if(subcategory_id === savedDocumentParse.subcategory_id){
                setDocumentELement(savedDocumentParse);
            }
        }
    ...
    useEffect(() => {
        localStorage.setItem('huquq-document', JSON.stringify(documentELement));
    }, [documentELement]);
        ...
        setDocumentELement((prevDocument) => {
            ...
            if (isChecked) {
                section.inputs = [
                    {
                        texts: { ...section.texts },
                        options: { ...section.options, type: 'text' },
    ...
    const handleCreateDocument = () => {
        dispatch(createDocument({form: documentELement, category_id: category_id}, navigate));
    };
